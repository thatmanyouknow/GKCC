...
def _managed_source_name(resolved: Path) -> Optional[str]:
    lowered = str(resolved).lower()
    markers = [
        ("/happy-hare/", "Happy Hare"),
        ("/mainsail-config/", "Mainsail configuration"),
        ("/moonraker-timelapse/", "Moonraker Timelapse"),
        ("/moonraker-obico/", "Obico"),
...
def config_path_metadata(path: str, root_status: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Inspect the real local path behind a Moonraker config-root entry.

    Moonraker intentionally rejects writes through symlinks into update-manager
    repositories. GKCC runs on the same host, so it can identify those paths
    before a backup or upload is attempted.
    """
    normalized = normalize_config_relpath(path)
    root_info = root_status or live_config_root_status()
    root_value = root_info.get("path") if isinstance(root_info, dict) else None
    result: Dict[str, Any] = {
        "path": normalized,
        "inspectable": False,
        "protected": False,
    ...
    if not root_value:
        result.update({
            "protected": True,
            "writable": False,
            "reason": "Moonraker did not report a local config-root path, so GKCC cannot verify whether this target is updater-managed.",
        })
        return result
    root_path = Path(str(root_value)).expanduser().resolve(strict=False)
    target = root_path.joinpath(*normalized.split("/"))
    cursor = root_path
    symlink_component: Optional[str] = None
    for part in normalized.split("/"):
        cursor = cursor / part
        try:
            if cursor.is_symlink():
                symlink_component = str(cursor.relative_to(root_path)).replace("\\", "/")
                break
        except OSError:
            break
    try:
    ...
    outside = not _path_is_within(resolved, root_path)
    managed_by = _managed_source_name(resolved)
    protected = bool(symlink_component or outside)
    result.update({
        "inspectable": True,
        "protected": protected,
        ...
        "outside_config_root": outside,
        "managed_by": managed_by,
    })
    if protected:
        owner = f" and is managed by {managed_by}" if managed_by else ""
        component = f" through symlink {symlink_component}" if symlink_component else ""
        destination = "outside the Moonraker config root" if outside else "through a symlinked config path"
        result["reason"] = (
            f"{normalized} resolves{component} {destination}{owner}. "
            "Moonraker reserves this path against file-manager writes."
        )
    return result


def _moonraker_http_error_detail(exc: urllib.error.HTTPError) -> str:
    try:
        raw = exc.read()
    except Exception:
        raw = b""
    ...
    if text:
        try:
            ...
            pass
        return text[:2000]
    return str(getattr(exc, "reason", "Moonraker rejected the request"))
...
def moonraker_upload_config(path: str, content: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        detail = _moonraker_http_error_detail(exc)
        raise RuntimeError(f"Moonraker HTTP {exc.code} while uploading {normalized}: {detail}") from exc
...
def build_query(objects: List[str], include_config: bool = False) -> Dict[str, Any]:
    ...
    for name in ("webhooks", "print_stats", "toolhead", "gcode_move", "extruder", "heater_bed", "probe", "motion_report"):
        if name in objects:
            requested[name] = None
    for name in objects:
        low = name.lower()
    ...
    try:
        ...
        for name, value in status.items():
            ...
            if "sensor" in low or "encoder" in low or low.startswith("gcode_button "):
                sensors[name] = value
        endstops: Dict[str, Any] = {}
        if "query_endstops" in reply["objects"]:
            try:
                raw_endstops = moonraker_request("/printer/query_endstops/status", timeout=10.0)
                result_endstops = unwrap_result(raw_endstops)
                if isinstance(result_endstops, dict):
                    endstops = {str(k): v for k, v in result_endstops.items()}
            except Exception:
                endstops = {}
...
def safe_runtime_stepper_names() -> List[str]:
    """Return steppers reported by Klipper's live motion_report object."""
    with _state_lock:
        report = copy.deepcopy(_state.get("motion_report", {}))
    values = report.get("steppers", []) if isinstance(report, dict) else []
    return sorted({str(item) for item in values if re.fullmatch(r"[A-Za-z0-9_]+", str(item))})


def hardware_status_snapshot() -> Dict[str, Any]:
    refresh_status()
    with _state_lock:
        state = copy.deepcopy(_state)
    return {
        "sensors": state.get("sensors", {}),
        "endstops": state.get("endstops", {}),
        "probe": state.get("probe", {}),
        "mmu": state.get("mmu", {}),
        "available_objects": state.get("available_objects", []),
        "motion_steppers": safe_runtime_stepper_names(),
        "updated_at": state.get("last_update"),
    }


def run_hardware_test(data: Dict[str, Any]) -> Dict[str, Any]:
    """Execute one guarded diagnostic action and return a fresh I/O snapshot."""
    ...
    if action == "refresh_inputs":
        commands: List[str] = []
        with _state_lock:
            objects = set(str(item) for item in _state.get("available_objects", []))
        if "query_endstops" in objects:
            commands.append("QUERY_ENDSTOPS")
        if "probe" in objects:
            commands.append("QUERY_PROBE")
        if any(name == "mmu" or name.startswith("mmu_") for name in objects):
            commands.append("MMU_SENSORS")
        if commands:
            script = "\n".join(commands)
            run_gcode(script, "Refresh live switch states")
        return {"ok": True, "script": script, "hardware": hardware_status_snapshot()}
    if action == "mmu_servo_position":
        pos = str(data.get("position", "")).lower()
        if pos not in {"up", "down", "move"}:
            raise ValueError("MMU servo position must be up, down, or move")
        script = f"MMU_SERVO POS={pos}"
        title = f"MMU servo {pos}"
    elif action == "mmu_servo_angle":
        angle = float(data.get("angle", -1))
        if not 0.0 <= angle <= 180.0:
            raise ValueError("MMU servo angle must be between 0 and 180 degrees")
        if str(data.get("confirmation", "")) != "SERVO":
            raise ValueError("Type SERVO to confirm direct angle movement")
        script = f"MMU_SERVO ANGLE={angle:.1f}"
        title = f"MMU servo angle {angle:.1f}"
    elif action == "mmu_motor_buzz":
        motor = str(data.get("motor", "")).lower()
        if motor not in {"gear", "selector", "servo"}:
            raise ValueError("MMU buzz motor must be gear, selector, or servo")
        script = f"MMU_TEST_BUZZ_MOTOR MOTOR={motor}"
        title = f"MMU {motor} buzz test"
    elif action == "mmu_motors_off":
        script = "MMU_MOTORS_OFF"
        title = "Release MMU motors and servos"
    elif action == "mmu_home_selector":
        if str(data.get("confirmation", "")) != "HOME":
            raise ValueError("Type HOME to confirm selector homing")
        script = "MMU_HOME"
        title = "Home MMU selector"
    elif action == "mmu_select_gate":
        gate = int(data.get("gate", -1))
        if not 0 <= gate <= 31:
            raise ValueError("Gate must be between 0 and 31")
        if str(data.get("confirmation", "")) != "SELECT":
            raise ValueError("Type SELECT to confirm selector movement")
        script = f"MMU_SELECT GATE={gate}"
        title = f"Select MMU gate {gate}"
    elif action == "klipper_stepper_buzz":
        stepper = str(data.get("stepper", "")).strip()
        allowed = safe_runtime_stepper_names()
        if stepper not in allowed:
            raise ValueError("Stepper is not present in Klipper motion_report")
        if str(data.get("confirmation", "")) != "BUZZ":
            raise ValueError("Type BUZZ to confirm stepper identification movement")
        script = f"STEPPER_BUZZ STEPPER={stepper}"
        title = f"Buzz {stepper}"
    else:
        raise ValueError("Unknown hardware diagnostic action")
    result = run_gcode(script, title)
    return {"ok": True, "script": script, "result": result, "hardware": hardware_status_snapshot()}
...
def normalize_builder_project(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    file_metadata = live_in.get("file_metadata", {})
    if isinstance(file_metadata, dict):
        for raw_path, raw_meta in list(file_metadata.items())[:200]:
            try:
                cfg_path = normalize_config_relpath(str(raw_path))
            except ValueError:
                continue
            if not isinstance(raw_meta, dict):
                continue
            live["file_metadata"][cfg_path] = {
                "path": cfg_path,
                "inspectable": bool(raw_meta.get("inspectable")),
                "protected": bool(raw_meta.get("protected")),
                "writable": bool(raw_meta.get("writable", not raw_meta.get("protected"))),
                "symlink_component": str(raw_meta.get("symlink_component"))[:500] if raw_meta.get("symlink_component") else None,
                "resolved_path": str(raw_meta.get("resolved_path"))[:1000] if raw_meta.get("resolved_path") else None,
                "outside_config_root": bool(raw_meta.get("outside_config_root")),
                "managed_by": str(raw_meta.get("managed_by"))[:200] if raw_meta.get("managed_by") else None,
                "reason": str(raw_meta.get("reason"))[:2000] if raw_meta.get("reason") else None,
            }
...
def rebase_project_from_live(project: Dict[str, Any], paths: Optional[List[str]] = None, preserve_edits: bool = True) -> Dict[str, Any]:
    requested = paths or [
        "printer.cfg",
    ...
    # Preserve draft configuration edits only when explicitly requested. A clean
    # rebase is useful after a failed/mixed-version project imported unintended
    # changes into updater-managed files. Workflow progress, notes, and taught
    # positions are preserved in both modes.
    if preserve_edits:
    ...
    baseline["taught_locations"] = copy.deepcopy(current.get("taught_locations", []))
    ...
    baseline.setdefault("live", {}).setdefault("file_metadata", {})
    for cfg_path in names:
        baseline["live"]["file_metadata"][cfg_path] = config_path_metadata(cfg_path, root_status)
    protected = [
        meta for meta in baseline["live"]["file_metadata"].values()
        if isinstance(meta, dict) and meta.get("protected")
    ]
    if protected:
        baseline["import_warnings"].append(
            f"{len(protected)} linked or externally managed configuration file(s) were loaded read-only."
        )
...
def project_change_sets(project: Dict[str, Any], apply_deletions: bool) -> Dict[str, Dict[str, Any]]:
    """Return only files with intentional section/include changes.

    v0.5.0 created an empty change-set entry for every parsed section. Even
    when no option changed, passing a file through the patcher could normalize
    its final newline and make it appear changed. That caused unrelated files,
    including updater-managed symlinks, to enter the transaction. This version
    does not create a target entry until a real update or approved deletion is
    present.
    """
    live = project.get("live", {}) if isinstance(project.get("live", {}), dict) else {}
    ...
    def target_change(section: str) -> Dict[str, Any]:
        target = normalize_config_relpath(section_files.get(section) or canonical_target_for_section(section))
        return changes.setdefault(target, {"updates": {}, "delete_options": {}, "delete_sections": []})
    ...
    for section in all_sections:
        ...
        if current_options is None:
            if apply_deletions and section in baseline_sections:
                target_change(section)["delete_sections"].append(section)
            continue
        ...
        if updates:
            target_change(section)["updates"][section] = updates

        if apply_deletions:
            removed = [option for option in baseline_options if option not in current_options]
            if removed:
                target_change(section)["delete_options"][section] = removed
...
def build_live_plan(project: Dict[str, Any], apply_deletions: bool = False) -> Dict[str, Any]:
    ...
    def append_candidate(path: str, current: Optional[bytes], candidate: bytes, mode: str) -> None:
        before = current or b""
        if candidate == before:
            return
        metadata = config_path_metadata(path, root_status)
        entry = {
            "path": path,
            ...
            "blocked": bool(metadata.get("protected")),
            "path_metadata": metadata,
            "protection_reason": metadata.get("reason"),
        }
        entries.append(entry)
        if entry["blocked"]:
            conflicts.append(str(metadata.get("reason") or f"{path} is protected from live writes"))
    ...
    if source_files:
        ...
        for path in candidate_paths:
            ...
            append_candidate(path, current, candidate, mode)
    else:
        mode = "full_replace"
    ...
    return {
        ...
        "blocked_files": [entry["path"] for entry in entries if entry.get("blocked")],
        "writable_changed_files": [entry["path"] for entry in entries if not entry.get("blocked")],
    }
...
def restore_backup_files(backup_id: str, only_paths: Optional[List[str]] = None) -> Dict[str, Any]:
    path, manifest = read_backup_manifest(backup_id)
    selected = {normalize_config_relpath(item) for item in only_paths} if only_paths is not None else None
    restored: List[str] = []
    skipped: List[str] = []
    root_status = live_config_root_status()
    with zipfile.ZipFile(path, "r") as archive:
        for item in manifest.get("files", []):
            ...
            if selected is not None and cfg_path not in selected:
                skipped.append(cfg_path)
                continue
            metadata = config_path_metadata(cfg_path, root_status)
            if metadata.get("protected"):
                raise RuntimeError(str(metadata.get("reason") or f"{cfg_path} is protected from restore"))
            if item.get("existed"):
    ...
    return {"ok": True, "backup_id": backup_id, "restored": restored, "skipped": skipped}
...
def apply_live_plan(
    ...
    try:
        ...
        if plan["conflicts"]:
            detail = " | ".join(str(item) for item in plan["conflicts"][:6])
            raise RuntimeError(f"Live apply is blocked: {detail}")
        ...
        try:
            ...
            restore_result: Dict[str, Any] = {
                ...
                "restored": [],
                "skipped": [],
                "message": "No files had been written, so rollback was not required.",
            }
            rollback_error: Optional[str] = None
            rolled_back = False
            if written:
                try:
                    restore_result = restore_backup_files(backup["backup_id"], only_paths=written)
                    rolled_back = bool(restore_result.get("ok"))
                except Exception as rollback_exc:
                    rollback_error = str(rollback_exc)
                    restore_result = {"ok": False, "restored": [], "error": rollback_error}
            add_record({
                "workflow_id": "live_config_apply",
                "title": "Live configuration upload failed",
                ...
                "notes": "GKCC restored only files successfully written by this transaction. No-op and protected files were not touched.",
            })
            rollback_note = (
                "Written files were restored from the transaction backup."
                if rolled_back else
                "No rollback was required because no files were written."
                if not written else
                "Automatic rollback failed; inspect the reported rollback error before restarting Klipper."
            )
            return {
                "ok": False,
                "rolled_back": rolled_back,
                "rollback_required": bool(written),
                "backup_id": backup["backup_id"],
                "changed_files": len(written),
                "files": written,
                "files_written_before_failure": written,
                "failed_file": failed_file,
                "moonraker_error": str(exc),
                "rollback": restore_result,
                "rollback_error": rollback_error,
                "error": f"Live upload failed at {failed_file or 'transaction validation'}: {exc}. {rollback_note}",
            }
        ...
        if restart:
            ...
            if not restart_result.get("ok"):
                ...
                return {
                    ...
                    "restore": restore_result,
                    "rollback_restart": rollback_restart,
                    "error": "Klipper rejected the proposed configuration; only files written by this transaction were restored automatically.",
                }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/hardware/status":
            self.send_json({"ok": True, "hardware": hardware_status_snapshot()})
            return
        if path == "/api/config":
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/builder/live/rebase":
                ...
                project = rebase_project_from_live(
                    project,
                    paths,
                    preserve_edits=bool(data.get("preserve_edits", True)),
                )
                atomic_write_json(BUILDER_PROJECT_PATH, project)
                self.send_json({
                    "ok": True,
            ...
            if path == "/api/live/restore":
                ...
                for item in manifest.get("files", []):
                    ...
                    metadata = config_path_metadata(cfg_path, root_status)
                    if metadata.get("protected"):
                        protected_targets.append(str(metadata.get("reason") or cfg_path))
                        continue
                ...
                if protected_targets:
                    raise RuntimeError(
                        "Backup restore is blocked because it contains updater-managed paths: "
                        + " | ".join(protected_targets[:6])
                    )
                ...
                return
            if path == "/api/action/emergency_stop":
                if not self.require_token():
                    return
                result = moonraker_request("/printer/emergency_stop", method="POST", payload={}, timeout=10.0)
                with _state_lock:
                    _state["last_action"] = "Emergency stop"
                    _state["last_result"] = "Requested immediately through Moonraker"
                self.send_json({"ok": True, "result": result})
                return
            if path == "/api/action/hardware_test":
                if not self.require_token():
                    return
                data = self.read_json()
                self.send_json(run_hardware_test(data))
                return
